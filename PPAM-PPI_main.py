...
with open("args_parameter_PPAM-PPI.yaml") as f:
    args.__dict__.update(yaml.safe_load(f))

# in-cluster
train_dataset = Class_CSVDataset('dataset/sample_train.csv',\
...
valid_dataset = Class_CSVDataset('dataset/sample_valid.csv',\
                                  'source','target', 'species', 'class','dataset/Eukaryotes_protein_len50_reportset_fasta.h5')
...
if args.validate:
    ...
    #trainer.fit(model, train_loader, valid_loader, ckpt_path="650M_freeze_classify/epoch=1-step=13210.ckpt") # continue training
